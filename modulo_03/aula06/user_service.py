...
class UserService:
    ...
    def register_user(
        ...
        if len(email) < 10 and "@" not in email and not email.endswith(".com"):
            ...
            return False, "Erro: E-mail inválido. Deve conter '@' e terminar com '.com'."
        
        if not nome_completo.replace(" ", "").isalpha():

            return False, "Erro: Nome completo deve conter apenas letras e não pode estar vazio."
        
        senha_hash = hash_senha(senha)

        success, message = self.UserModel.create_user(senha_hash, email, nome_completo, perfil)
    ...
    def login_user(self, email: str, senha: str) -> tuple[dict | None, str]:
        ...
        user = self.UserModel.find_user_by_email(email)
        ...
        return None, "Erro: E-mail ou senha incorretos."    
